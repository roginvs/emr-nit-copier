    ...
    for card in destinations.keys():
      ...
      for channel in destinations[card]:
        ...
        if psiPkt:
          ...
          pktNum = root.find('qam8PortPara').find('idx%s' % channel).find('pktNum').text.split(',')
          if int(pktNum[4]) != len(source_nit):
              sys.stdout.write(u'      Корректируем pktNum.. ')
              pktNum[4] = "%s" % len(source_nit)
              root.find('qam8PortPara').find('idx%s' % channel).find('pktNum').text = ','.join(pktNum)
              sys.stdout.write(u'ok\n')
...
delete_temp() 
sys.stdout.write(u'Всё готово, нужно ничего в настройках не меняя перезагрузить EMR!\n')
